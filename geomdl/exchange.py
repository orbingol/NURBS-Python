...
def export_yaml(obj, file_name):
    ...
    Unfortunately, it doesn't work with multi curves ans surfaces at the moment.
    ...
    try:
        with open(file_name, 'w') as fp:
            ...
            if isinstance(obj, Abstract.Curve):
                yaml_str += _prepare_export_yaml_curve(obj)

            else:
                yaml_str += _prepare_export_yaml_surface(obj)
            fp.write(yaml_str)
    except IOError as e:
        print("An error occurred: {}".format(e.args[-1]))
        raise e
