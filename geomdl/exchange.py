...
def export_yaml(obj, file_name):
    ...
    :raises IOError: an error occurred writing the file
    """
    yaml_str = "# Generated by geomdl\n"
    yaml_str += "# file: " + file_name + "\n\n"

    # Generate YAML file contents
    if isinstance(obj, Abstract.Curve):
        yaml_str += _export_yaml_curve_single(obj)
    elif isinstance(obj, Abstract.Surface):
        yaml_str += _export_yaml_surface_single(obj)
    elif isinstance(obj, Multi.MultiCurve):
        yaml_str += _export_yaml_curve_multi(obj)
    elif isinstance(obj, Multi.MultiSurface):
        yaml_str += _export_yaml_surface_multi(obj)
    else:
        raise NotImplementedError("Cannot export " + obj.__class__.__name__ + " type in yaml format")
...
def _prepare_export_yaml_curve(obj):
    ...
    try:
        ...
        # obj must be an instance of B-Spline class, so don't do anything
        pass
    line += "      delta: " + str(obj.delta) + "\n"
    return line


def _prepare_export_yaml_surface(obj):
    line = ""
    line += "    - degree_u: " + str(obj.degree_u) + "\n"
    line += "      degree_v: " + str(obj.degree_v) + "\n"
    line += "      knotvector_u: [" + ", ".join(str(kv) for kv in obj.knotvector_u) + "]\n"
...
def _export_yaml_curve_single(obj):
    line = "shape:\n"
    line += "  type: curve\n"
    line += "  data:\n"
    line += _prepare_export_yaml_curve(obj)
    return line


def _export_yaml_surface_single(obj):
    line = "shape:\n"
    line += "  type: surface\n"
    line += "  data:\n"
    line += _prepare_export_yaml_surface(obj)
    return line


def _export_yaml_curve_multi(multi_obj):
    line = "shape:\n"
    line += "  type: curve\n"
    line += "  data:\n"
...
def _export_yaml_surface_multi(multi_obj):
    line = "shape:\n"
    line += "  type: surface\n"
    line += "  data:\n"
    for obj in multi_obj:
