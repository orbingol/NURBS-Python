...
def _export_cfg_curve(obj, file_name):
    try:
        with open(file_name, 'w') as fp:
            ...
            fp.write(_prepare_cfg_curve(obj))
    except IOError as e:
        print("An error occurred: {}".format(e.args[-1]))
        raise e
    except Exception:
...
def _export_cfg_surface(obj, file_name):
    try:
        with open(file_name, 'w') as fp:
            # File header
            fp.write("# Generated by NURBS-Python\n")
            ...
            # Write object properties
            fp.write(_prepare_cfg_surface(obj))
    except IOError as e:
        print("An error occurred: {}".format(e.args[-1]))
        raise e
    except Exception:
...
def _prepare_cfg_curve(obj, idx=0):
    ...
    # Finish exporting object
    line += "};\n"

    return line


def _prepare_cfg_surface(obj, idx=0):
    """ Prepares surface object for libconfig-type export.

    :param obj: surface object
    :param idx: surface index (for multi surfaces)
    :return: surface data as a string
    """
    line = ""

    # Start exporting object
    line += "surface" + (str(idx) if idx > 0 else "") + ": \n{\n"
    line += "\ttype = \"surface\";\n"
