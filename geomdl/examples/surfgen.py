...
class Grid(object):
    ...
    def generate(self, num_u, num_v):
        ...
        if not isinstance(num_u, int):
            num_u = int(num_u)
            base.GeomdlWarning("%d will be used as the value of num_u" % num_u, UserWarning)

        if not isinstance(num_v, int):
            num_v = int(num_v)
            base.GeomdlWarning("%d will be used as the value of num_v" % num_v, UserWarning)
    ...
    def bumps(self, num_bumps, **kwargs):
        ...
        * ``base_adjust``: padding between the bases of the hills. *Default: 0*
        ...
        if not isinstance(num_bumps, int):
            ...
            base.GeomdlWarning("Number of bumps must be an integer value. Automatically rounding to %d" % num_bumps,
                          UserWarning)

        if isinstance(bump_height, base.GeomdlTypeSequence):
...
class GridWeighted(Grid):
    ...
    def weight(self, value):
        ...
        elif isinstance(value, base.GeomdlTypeSequence):
