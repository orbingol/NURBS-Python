...
process. NURBS-Python uses :py:func:`.export_cfg()` and ``import_cfg`` functions to exporting and importing shape data
which can be processed by libconfig-compatible libraries. Although exporting does not require any external libraries,
importing functionality depends on `libconf <https://pypi.org/project/libconf/>`_ module which is a pure Python library
for parsing libconfig format.
